...
class AgoraClubController: BaseViewController {
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        SyncUtil.scene(id: channelName)?.collection(className: SYNC_MANAGER_AGORA_CLUB_USERS).delete(id: currentUserModel?.objectId ?? "", success: {}, fail: { error in
                ...
                self.liveToolView.onTapItemClosure = { itemType, isSelected in
                    ...
                    case .camera:
                        ...
                        if var userModel = self.currentUserModel {
                            ...
                            SyncUtil.scene(id: self.channelName)?.collection(className: SYNC_MANAGER_AGORA_CLUB_USERS).update(id: userModel.objectId ?? "", data: params, success: {}, fail: { error in
                                ToastView.show(text: error.message)
                            })
                            self.currentUserModel = userModel
                        }
