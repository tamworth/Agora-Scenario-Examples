...
class EducationCreateController: BaseViewController {
    lazy var service: EducationService = EducationService()
    ...
    private func onTapStartLiveButton() {
        service.join(roomName: randomNameView.text) {[weak self] (error, resp) in
            ...
            let livePlayerVC = EducationController(channelName: resp.channelName,
                                                   userId: resp.userId,
