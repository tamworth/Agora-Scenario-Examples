...
extension CDNRoomListViewCell {
    ...
    class LabelShadowView: UIView {
        ...
        override func layoutSubviews() {
            ...
            let shadowW: CGFloat = bounds.width - (shadowOffSetX * 2)
            let shadowX: CGFloat = shadowOffSetX
            let shadowY: CGFloat = bounds.height - shadowH
            shadow.frame = CGRect(x: shadowX,
                                  y: shadowY,
                                  width: shadowW,
                                  height: shadowH)

            let labelH: CGFloat = 25.0
            let labelW: CGFloat = bounds.width - (shadowOffSetX * 2)
            let labelX: CGFloat = shadowOffSetX
            let labelY: CGFloat = bounds.height - labelH
