...
class LivePKController: BaseViewController {
    ...
    public func pkSubscribeHandler(object: IObject) {
        ...
        } else if model.status == .invite && model.userId != "\(UserInfo.userId)" {
        ...
        } else if model.status == .refuse && model.userId == "\(UserInfo.userId)" {
    ...
    public func joinBroadcasterChannel(channelName: String, uid: UInt) {
        ...
        if getRole(uid: "\(uid)") == .broadcaster && channelName == channleName {
