//
//  EducationRoomListController.swift
//  Scene-Examples
//
...
@objc
class EducationRoomListController: BaseViewController {
    ...
    private func getLiveData() {
        guard syncUtil.inited else {
            syncUtil.initSyncManager(sceneId: "EducationRoomListController") { [weak self] in
...
//        }
        ...
        createLiveVC.syncUtil = syncUtil
    ...
    private func joinSceneHandler(result: IObject) {
        ...
        let vc = EducationController(channelName: channelName ?? "",
                                      userId: ownerId ?? "")
        vc.syncUtil = syncUtil
        navigationController?.pushViewController(vc, animated: true)

    }
}

extension EducationRoomListController: AGETableViewDelegate {
